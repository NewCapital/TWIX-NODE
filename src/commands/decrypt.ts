...
The main use case of this command is to verify private keys in encrypted files after encrypting a custom preset or running a bootstrap command with a provided --password.`;
    ...
    static examples = [
        `
$ twix-node start --password 1234 --preset testnet --assembly dual --customPreset decrypted-custom-preset.yml --detached
...
$ rm plain-custom-preset.yml`,
        `
$ echo "$MY_ENV_VAR_PASSWORD" | twix-node decrypt --source target/addresses.yml --destination plain-addresses.yml
`,
    ];
