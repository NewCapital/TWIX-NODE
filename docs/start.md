...
* [`twix-node start`](#twix-node-start)
...
EXAMPLES
  $ twix-node start
  $ twix-node start -p bootstrap
  $ twix-node start -p testnet -a dual
  $ twix-node start -p testnet -a dual --password 1234
  $ echo "$MY_ENV_VAR_PASSWORD" | twix-node start -p testnet -a dual
```

_See code: [src/commands/start.ts](https://github.com/NewCapital/TWIX-NODE/src/commands/start.ts)_
